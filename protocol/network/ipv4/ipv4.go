...
func (e *endpoint) WritePacket(r *stack.Route, hdr buffer.Prependable, payload buffer.VectorisedView,
	...
	// 写入网卡接口
	// log.Printf("@网络层 ipv4: 接受传输层数据 封装ip头部信息，写入链路层网卡数据", hdr.UsedLength()+payload.Size(), protocol)
	tmp := [][]byte{hdr.View(), payload.First()}
	log.Println("@网络层 ipv4: 接受传输层数据 封装ip头部信息，写入链路层网卡数据 ", bytes.Join(tmp, []byte("")))
