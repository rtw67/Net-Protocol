...
func (e *endpoint) WritePacket(r *stack.Route, hdr buffer.Prependable, payload buffer.VectorisedView, protocol tcpip.NetworkProtocolNumber) *tcpip.Error {
	// tmp := [][]byte{hdr.View(), payload.First()}
	// log.Println("@链路层: fdbased 写入网卡数据 :", bytes.Join(tmp, []byte("")))
	...
	eth.Encode(ethHdr)
	// log.Println("@链路层: fdbased 写入网卡数据 :", eth)
	tmp := [][]byte{hdr.View(), payload.First()}
	remoteLinkAddr := eth.SourceAddress()
	localLinkAddr := eth.DestinationAddress()
	log.Println("@链路层: fdbased 写入网卡数据 :", bytes.Join(tmp, []byte("")), localLinkAddr, remoteLinkAddr)
	//写入网卡中
	if payload.Size() == 0 {
...
func (e *endpoint) dispatch() (bool, *tcpip.Error) {
	...
	// log.Printf("@链路层 fdbased: step1 物理网卡接受数据:read %d bytes", n)
	if err != nil {
		return false, err
	}
	//如果比头部长度还小，直接丢弃
	...
	remoteLinkAddr = eth.SourceAddress()
	localLinkAddr = eth.DestinationAddress()
	log.Println("@链路层 fdbased: step2 解析以太网协议:", eth, localLinkAddr, remoteLinkAddr)
